...
"""
Date: Feb 16, 2026
Description: Compute TE adjacency matrices and persistence diagrams from evaluation spike data (logic-gate experiments), then summarize topology via Betti curves and AUBC.
Inputs:
  - Experiment root containing evaluation/test_*/activity/ spike arrays and evaluation_accuracy_log.csv.
  - Command-line arguments selecting TE history length k, homology dimensions, and grouping modes.
Outputs:
...
def parse_args():
    ...
    ap.add_argument("--root",  default="./and_or_xor_2bit-20_N-64_P_HIGH-5_P_LOW-0_2",
    ...
    ap.add_argument("--modes", nargs="+", default=["and","or","xor"],
...
def compute_or_load_pd(adj, pd_path, dims, force):
    if force or not os.path.exists(pd_path):
        ...
        pd = compute_persistence_diagram(adj, homology_dimensions=dims, directed=directed)
...
def main():
    ...
    for td in sorted(d for d in os.listdir(eval_dir) if d.startswith("test_")):
        ...
        gate = label.rsplit("_", 1)[-1]  # 'and' / 'or' / 'xor'
        if gate in auc_by_mode:
            for d in dims:
                auc_by_mode[gate][d].append(auc_vals[d])
            pds_by_mode[gate].append(pd)
